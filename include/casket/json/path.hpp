...
public:
    ...
    template<size_t N>
    Path(const std::array<std::string_view, N>& segments)
    {
    ...
    Path operator/(std::string_view segment) const
    {
        Path result = *this;
        result /= segment;
        return result;
    ...
    std::string_view operator[](size_t index) const { return parts_[index]; }
    ...
    std::string_view front() const { return parts_[0]; }
    std::string_view back() const { return parts_[count_ - 1]; }
    ...
    {
        return count_ == 0 ? std::string_view{} : parts_[count_ - 1];
    }
    
    auto begin() const { return parts_.begin(); }
    auto end() const { return parts_.begin() + count_; }
...
private:
    ...
    void parse(std::string_view path)
    {
        ...
        while (end != std::string_view::npos && count_ < MAX_DEPTH)
        {
    ...
    std::array<std::string_view, MAX_DEPTH> parts_{};
    size_t count_ = 0;
};

inline Path operator/(std::string_view left, std::string_view right)
{
    Path p(left);
    p /= right;
    return p;
