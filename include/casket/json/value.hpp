...
struct Array {
    ...
    Array(std::initializer_list<Value> il) : data_(il) {}
    
    Value& operator[](size_t idx) { return data_[idx]; }
    const Value& operator[](size_t idx) const { return data_[idx]; }
    size_t size() const noexcept { return data_.size(); }
    bool empty() const noexcept { return data_.empty(); }
    
    template <typename T>
    void push_back(T&& value) { data_.push_back(std::forward<T>(value)); }
    ...
    std::vector<Value>&& data() && noexcept { return std::move(data_); }
};

// ============================================================================
// Object - только объявления методов
// ============================================================================
struct Object {
...
public:
    ...
    Variant&& variant() && noexcept { return std::move(data_); }
    ...
    bool isNull() const noexcept { return std::holds_alternative<Null>(data_); }
};

// ============================================================================
// Value::as специализации (объявления)
// ============================================================================
template <> std::optional<std::string> Value::as<std::string>() const noexcept;
template <> std::optional<Integer> Value::as<Integer>() const noexcept;
template <> std::optional<int> Value::as<int>() const noexcept;
template <> std::optional<Float> Value::as<Float>() const noexcept;
template <> std::optional<Boolean> Value::as<Boolean>() const noexcept;
template <> std::optional<Null> Value::as<Null>() const noexcept;
