...
inline typename std::enable_if<!std::is_member_pointer<To>::value && !std::is_reference<To>::value &&
...
aggressive_ptr_cast(From v) noexcept
{
    ...
    static_assert(std::is_void<typename std::remove_pointer<To>::type>::value ||
...
template <class To, class From>
inline typename std::enable_if<std::is_reference<To>::value && !std::is_member_pointer<From>::value, To>::type
aggressive_ptr_cast(From v) noexcept
{
    static_assert(std::is_pointer<From>::value, "`agressive_ptr_cast` function must be used only for pointer casting.");

    static_assert(std::is_void<typename std::remove_pointer<From>::type>::value,
                  "`agressive_ptr_cast` function must be used only for casting to or from void pointers.");
...
template <class To, class From>
inline typename std::enable_if<std::is_member_pointer<To>::value && !std::is_member_pointer<From>::value, To>::type
aggressive_ptr_cast(From v) noexcept
{
    static_assert(std::is_pointer<From>::value, "`agressive_ptr_cast` function must be used only for pointer casting.");
...
aggressive_ptr_cast(From /* v */) noexcept
{
    static_assert(std::is_pointer<To>::value, "`agressive_ptr_cast` function must be used only for pointer casting.");
