...
public:
    ...
    In in(const QuerySelect& sel) const;
...
public:
    ...
    const Field& operator=(T2 v) {
        _modified = true;
        _value = convert<T2, std::string>(v);
        return *this;
    }
    template <class T2>
    bool operator==(const T2& v) const {
...
template <class T>
std::string operator+(const std::string& a, const Field<T>& f) {
    return a + std::string(f);
}
template <class T>
std::string operator+(const Field<T>& f, const std::string& a) {
