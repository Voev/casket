...
TEST_F(PackerTest, PackString8)
{
    ...
    ASSERT_TRUE(result);
    ASSERT_EQ(packer_->size(), 2 + str.size());
...
TEST_F(PackerTest, PackString16)
{
    ...
    auto result = packer_->pack(nonstd::string_view(str));
    ASSERT_TRUE(result);
    ASSERT_EQ(packer_->size(), 3 + str.size());
...
TEST_F(PackerTest, PackString32)
{
    ...
    auto result = packer_->pack(nonstd::string_view(str));
    ASSERT_TRUE(result);
    ASSERT_EQ(packer_->size(), 5 + str.size());
...
{
    packer_->pack(42);
    size_t firstSize = packer_->size();
    ASSERT_GT(firstSize, 0);

    packer_->reset();
    ASSERT_EQ(packer_->size(), 0);

    packer_->pack("Hello");
    ASSERT_GT(packer_->size(), 0);
    ASSERT_NE(packer_->size(), firstSize);
}
...
TEST_F(PackerTest, MultiplePacks)
{
    ...
    ASSERT_EQ(packer_->size(), expectedSize);
}
...
TEST_F(PackerTest, CapacityAndRemaining)
{
    ...
    packer_->pack(42);
    ASSERT_EQ(packer_->remaining(), capacity - packer_->size());
}
...
TEST_F(PackerTest, ComplexStructure)
{
    ...
    ASSERT_EQ(packer_->size(), 3 + 5 + 3 + (2 + 4) + (2 + 3) + (2 + 3) + (2 + 1) + (2 + 1));
}
