...
TEST_F(JsonParserTest, ParseUnquotedString)
{
    Value result = parseDSL("hello");
    EXPECT_TRUE(result.is<std::string>());
    EXPECT_EQ(getString(result), "hello");
...
TEST_F(JsonParserTest, ParseScientificNotation)
{
...
TEST_F(JsonParserTest, ParseBooleanTrue)
{
...
TEST_F(JsonParserTest, ParseBooleanFalse)
{
...
{
    Value result = parseDSL("{ name: \"Alex\", age: 30 }");
    EXPECT_TRUE(result.is<Object>());

    auto* obj = getObject(result);
    ASSERT_NE(obj, nullptr);
    EXPECT_EQ(obj->fields.size(), 2);

    EXPECT_EQ(getString(obj->fields.at("name")), "Alex");
    EXPECT_EQ(getInteger(obj->fields.at("age")), 30);
}

TEST_F(JsonParserTest, ParseObjectWithoutComma)
{
    Value result = parseDSL("{ name: \"Alex\" age: 30 }");
    EXPECT_TRUE(result.is<Object>());
...
TEST_F(JsonParserTest, ParseNestedObjectWithoutComma)
{
    Value result = parseDSL("{ app: { name: \"myapp\" db: { host: \"localhost\" port: 5432 } } }");
    EXPECT_TRUE(result.is<Object>());
...
TEST_F(JsonParserTest, ParseArrayWithoutComma)
{
    Value result = parseDSL("[1 2 3]");
    EXPECT_TRUE(result.is<Array>());
...
TEST_F(JsonParserTest, ParseMixedArray)
{
...
TEST_F(JsonParserTest, ParseArrayOfObjectsWithoutComma)
{
    Value result = parseDSL("[{ name: \"Alex\", age: 30 } { name: \"Bob\", age: 25 }]");
    const auto* arr = getArray(result);
    ASSERT_NE(arr, nullptr);
...
TEST_F(JsonParserTest, ParseComplexNested)
{
    std::string input = R"(
        {
            config: {
...
TEST_F(JsonParserTest, ParseNestedEmptyObject)
{
    Value result = parseDSL("{ config: {} }");
    EXPECT_TRUE(result.is<Object>());
...
TEST_F(JsonParserTest, ParseObjectWithQuotedKeys)
{
    Value result = parseDSL("{ \"name\": \"Alex\", \"age\": 30 }");
    EXPECT_TRUE(result.is<Object>());
...
TEST_F(JsonParserTest, ParseObjectWithSingleQuotedKeys)
{
    Value result = parseDSL("{ 'name': 'Alex', 'age': 30 }");
    EXPECT_TRUE(result.is<Object>());
...
TEST_F(JsonParserTest, ParseErrorUnclosedBrace)
{
    EXPECT_THROW(parseDSL("{ name: \"Alex\" "), std::runtime_error);
}

TEST_F(JsonParserTest, ParseErrorUnclosedArray)
{
    EXPECT_THROW(parseDSL("[1, 2, 3"), std::runtime_error);
}

TEST_F(JsonParserTest, ParseErrorUnclosedQuote)
{
    EXPECT_THROW(parseDSL("{ text: \"hello }"), std::runtime_error);
}

TEST_F(JsonParserTest, ParseErrorMissingColon)
{
    EXPECT_THROW(parseDSL("{ name \"Alex\" }"), std::runtime_error);
}

TEST_F(JsonParserTest, ParseErrorExtraTokens)
{
