...
public:
    ...
    void setGenerationMode(generation_mode_t mode) {
        m_generationMode = mode;
    };
    generation_mode_t getGenerationMode() const {
    ...
    const CodeGenerator* getParentGenerator() const {
        return m_pParentGenerator;
    };
    void setParentGenerator(CodeGenerator* parent) {
    ...
    virtual bool generate(const xml::ObjectPtr&) {
        return true;
    };
    ...
    virtual bool generate(const xml::ObjectPtr&, std::ostream&, size_t) {
        return true;
    };
    virtual bool generate(const xml::Relation::Ptr&, std::ostream&, size_t) {
        return true;
    }
...
protected:
    CodeGenerator(const char* target)
        : m_target(target)
        , m_generationMode(REFRESH){};

protected:
    class AbstractFactory {
    public:
        AbstractFactory(const char* pszName)
            : name(pszName){};
...
public:
    ...
    CompositeGenerator(const char* target)
