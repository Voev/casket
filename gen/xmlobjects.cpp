...
using casket::utils::join;
using casket::storage::toString;
...
ObjectPtr Object::DEFAULT_BASE(new Object("Persistent", ""));

Field::Ptr Object::ID_FIELD(new Field("id", A_field_type_integer, "",
                                      A_field_indexed_true,
                                      A_field_unique_true));
Field::Ptr Object::TYPE_FIELD(new Field("type", A_field_type_string, "",
...
void generateRandomArray(uint8_t* array, size_t size)
{
    srand(time(0));
    for (size_t i = 0; i < size; i++)
    {
        array[i] = rand() % 255; // Генерируем случайное число от 0 до 99
    }
}
...
{
    uint8_t ss[20];
    generateRandomArray(ss, sizeof(ss));
    auto hex = casket::utils::hexlify({ss, sizeof(ss)});
    if (s.size() > 30)
        return "O" + hex.substr(0, 29);
    return s;
}

static void sanityCheck(DatabasePtr& db, const ObjectSequence& objects,
                        const Relation::sequence& relations)
{
    ...
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        if (usedID.find(o.name) != usedID.end())
            throw std::runtime_error("duplicate id: object.name : " + o.name);
        usedID[o.name] = true;
        ...
        for (size_t i2 = 0; i2 < o.fields.size(); i2++)
        {
            ...
            if (!(err = validID(f.name)).empty())
                throw std::runtime_error(
                    "invalid id: object.field.name : " + o.name + "." + f.name +
                    " cause:" + err);
            if (usedField.find(f.name) != usedField.end())
                throw std::runtime_error("duplicate id: object.field.name : " +
                                         o.name + "." + f.name);
    ...
    for (size_t i = 0; i < relations.size(); i++)
    {
        ...
        if (!(err = validID(name, "class")).empty())
            throw std::runtime_error("invalid id: relation.name : " + name +
                                     "cause:" + err);
        if (usedID.find(name) != usedID.end())
            throw std::runtime_error("duplicate id: relation.name : " + name);
        usedID[name] = true;
        map<string, bool> usedField;
        usedField.clear();
        ...
        for (size_t i2 = 0; i2 < r.fields.size(); i2++)
        {
            ...
            if (!(err = validID(f.name)).empty())
                throw std::runtime_error("invalid id: relation.field.name : " +
                                         name + "." + f.name + "cause:" + err);
            if (usedField.find(f.name) != usedField.end())
                throw std::runtime_error(
                    "duplicate id: relation.field.name : " + name + "." +
                    f.name);
            usedField[f.name] = true;
            if (f.default_.size() > 0)
                ...
                throw std::runtime_error(
                    "no default-value after field with default value : " +
                    name + "." + f.name);
        }
        usedField.clear();
        bool limits = false;
        ...
        for (size_t i2 = 0; i2 < r.related.size(); i2++)
        {
            ...
            if (!(err = validID(rel.handle)).empty() && !rel.handle.empty())
                throw std::runtime_error(
                    "invalid id: relation.relate.handle : " + name + "." +
                    rel.handle + "cause:" + err);
            if (usedField.find(rel.handle) != usedField.end())
                throw std::runtime_error(
                    "duplicate id: relation.relate.handle : " + name + "." +
                    rel.handle);
            if (objectName.find(rel.objectName) == objectName.end())
                throw std::runtime_error(
                    "unknown object: relation.relate.name : " + name + "." +
                    rel.objectName);
            if (!rel.handle.empty())
            ...
            if (uniques && limits)
                throw std::runtime_error(
                    "both 'unique' and 'limit' attributes used in relation " +
                    name);
        }
        if (r.related.size() != 2 && limits)
            throw std::runtime_error("'limit' attribute used in relation of " +
                                     toString(r.related.size()) +
                                     " object(s) " + name);
    }
}

static void initSchema(DatabasePtr& db, ObjectSequence& objects,
                       Relation::sequence& relations)
{
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        for (size_t i2 = 0; i2 < o.fields.size(); i2++)
        {
            ...
            fld->field->unique =
                (f->isUnique()) ? A_field_unique_true : A_field_unique_false;
        ...
        for (size_t i2 = 0; i2 < o.indices.size(); i2++)
        {
            ...
            for (size_t i3 = 0; i3 < idx.fields.size(); i3++)
            {
                if (fldMap.find(idx.fields[i3].name) == fldMap.end())
                    throw std::runtime_error("Indexfield " + o.name + "." +
                                             idx.fields[i3].name +
                                             " is invalid.");
            ...
            index->name =
                makeDBName(tbl->name + "_" + join(fldNames, "_") + "_idx");
    ...
    for (size_t i = 0; i < relations.size(); i++)
    {
        ...
        for (size_t i2 = 0; i2 < r.indices.size(); i2++)
        {
            ...
            for (size_t i3 = 0; i3 < idx.fields.size(); i3++)
            {
                //                Database::DBField* fld = new
                //                Database::DBField;
                if (fldMap.find(idx.fields[i3].name) == fldMap.end())
                    throw std::runtime_error("Indexfield " + r.name + "." +
                                             idx.fields[i3].name +
                                             " is invalid.");
                index->fields.push_back(fldMap[idx.fields[i3].name]);
                fldNames.push_back(idx.fields[i3].name);
...
void xml::init(DatabasePtr& db, ObjectSequence& objects,
               Relation::sequence& relations)
{
    ...
    for (Relation::sequence::const_iterator it_rel = relations.begin();
         it_rel != relations.end(); ++it_rel)
    {
        sort((*it_rel)->related.begin(), (*it_rel)->related.end(),
             Relate::CompareByObjectName());
    }
         ...
         it_rel != relations.end(); ++it_rel)
    {
        ...
        for (size_t i2 = 0; i2 < rel->related.size(); i2++)
        {
            ...
            if (!relate->handle.empty())
            {
                ...
                RelationHandle::Ptr handle(
                    new RelationHandle(relate->handle, rel, relate, obj));
                for (size_t i3 = 0; i3 < rel->related.size(); i3++)
                {
                    if (i2 != i3)
                    {
                        ...
                        handle->destObjects.push_back(
                            make_pair(o, rel->related[i3]));
                    }
                }
