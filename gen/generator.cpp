...
bool CodeGenerator::FactoryMap::registerFactory(AbstractFactory* pFactory)
{
    ...
    {
        pair<const char*, AbstractFactory*> value(pFactory->getName(),
                                                  pFactory);
        insert(value);
        return true;
    }
...
bool CodeGenerator::generate(const ObjectSequence& objects, ostream& os,
                             size_t indent)
{
...
bool CodeGenerator::generate(const Relation::sequence& relations, ostream& os,
                             size_t indent)
{
    for (Relation::sequence::const_iterator it = relations.begin();
         it != relations.end(); it++)
...
void CompositeGenerator::setOutputDirectory(const string& directory)
{
    ...
    for (vector<CodeGenerator*>::iterator it = generators.begin();
