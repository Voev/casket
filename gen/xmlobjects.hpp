...
inline std::string _capitalize(const std::string& s) {
    std::string result(s);
    if (!s.empty()) {
...
public:
    ...
    std::string length;
    Field(
        const std::string& n, AT_field_type t, const std::string& d, AT_field_indexed i, AT_field_unique u,
        const std::string& l = "")
        : name(n)
        , fieldTypeName(_capitalize(n))
        , type(t)
        , default_(d)
        , indexed(i)
        , unique(u)
...
public:
    ...
    std::string name;
    AT_param_type type;
    Param(const std::string& n, const AT_param_type& t)
        : name(n)
        , type(t) {
    }
};
...
public:
    ...
    struct CompareByObjectName : public std::binary_function<Ptr, Ptr, bool> { // functor for operator<
        bool operator()(const Ptr& _Left, const Ptr& _Right) const {           // apply operator< to operands
...
public:
    ...
    std::string getTable() const {
        ...
        return makeDBName(casket::utils::join(res, "_"));
    }
};
...
public:
    ...
    public:
        ...
        void name(const std::string& fieldname) {
    ...
    public:
        ...
        std::string getSQL() {
            ...
            if (unique)
                uniqueS = " UNIQUE";
            return "CREATE" + uniqueS + " INDEX " + name + " ON " + table + " (" + casket::utils::join(flds, ",") + ")";
        }
    };
        ...
        std::string getSQL(const std::string& rowIDType) {
            ...
            for (size_t i = 0; i < fields.size(); i++) {
                if (fields[i]->primaryKey)
                    ...
                    flds.push_back(
                        fields[i]->name() + " \" + backend->getSQLType(" + "A_field_type_" +
                        toAttributeString(fields[i]->field->type) + ",\"" + fields[i]->field->length + "\")" + " + \"" +
                        (fields[i]->field->isUnique() ? " UNIQUE" : "") + +"\" +" + "\"");
            }
            return "CREATE TABLE " + name + " (" + casket::utils::join(flds, ",") + ")";
        }
    };
...
void init(DatabasePtr& db, Object::sequence& objects, Relation::sequence& relations);
