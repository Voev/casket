...
public:
    ...
    bool isStatic, isVirtual, isProtected, isTemplate, isTemplateSpec,
        isDefinition, isConst, isAbstract;
    ...
    void write(FILE* hpp, FILE* cpp, string context, int indent = 0)
    {
        ...
        if (!isTemplateSpec)
            fprintf(hpp, "%s%s%s%s%s(%s)%s", ind.c_str(), tpl.c_str(),
                    sta.c_str(), ret.c_str(), name.c_str(), paramString.c_str(),
                    cnst.c_str());
        if (isDefinition)
            fprintf(hpp, ";\n");
        else
...
static bool validID(const string& s)
{
    static const char* words[] = {
        "asm", "break", "case", "catch", "char", "class", "const", "continue",
        ...
        "void", "volatile", "while",
        "initValues", "insert", "addUpdates", "addIDUpdates", "getFieldTypes",
        "delRecord", "delRelations", "update", "del", "typeIsCorrect", "upcast",
        "upcastCopy"};
...
{
    if (!validID(db->name))
        throw std::runtime_error("invalid id: database.name : " + db->name);
    for (size_t i = 0; i < objects.size(); i++)
    {
        xml::Object& o = *objects[i];
        if (!validID(o.name))
            throw std::runtime_error("invalid id: object.name : " + o.name);
        for (size_t i2 = 0; i2 < o.fields.size(); i2++)
        {
            xml::Field& f = *o.fields[i2];
            if (!validID(f.name))
                throw std::runtime_error("invalid id: object.field.name : " +
                                         f.name);
        }
    }
    for (size_t i = 0; i < relations.size(); i++)
    {
        ...
        if (!validID(r.getName()))
            throw std::runtime_error("invalid id: relation.name : " +
                                     r.getName());
        for (size_t i2 = 0; i2 < r.fields.size(); i2++)
        {
            xml::Field& f = *r.fields[i2];
            if (!validID(f.name))
                throw std::runtime_error("invalid id: relation.field.name : " +
                                         f.name);
        }
        for (size_t i2 = 0; i2 < r.related.size(); i2++)
        {
            ...
            if (!validID(rel.handle) && !rel.handle.empty())
                throw std::runtime_error(
                    "invalid id: relation.relate.handle : " + rel.handle);
        }
    }
}
...
static void writeStaticObjData(Class& cl, const xml::Object& o)
{
    ...
    Variable table__("table__", "const std::string", quote(o.getTable()));
    table__.static_();
    cl.variable(table__);
...
static void writeObjFieldValues(const xml::Object& o, const Field& fld,
                                Class& cl, Method& initValues)
{
    string data = quote(fld.name + "_") + "," + "A_field_type_" +
                  toAttributeString(fld.type) + "," + "table__";
    if (!fld.values.empty())
    {
        ...
        Class ftypeCl(ftypeClass, "FieldType");
        ...
        cons.param(Variable("n", "const std::string&"))
            ...
            .param(Variable("vals", "const FieldType::Values&", "Values()"))
            .constructor("FieldType(n,t,tbl,vals)");
...
static void writeObjFields(Class& cl, const xml::Object& o)
{
    ...
    string ftypeClass = "const FieldType";
    ...
    Variable ftype(
        "Id", ftypeClass,
        quote("id_") + "," + "A_field_type_" +
            toAttributeString(A_field_type_integer) //+ quote("INTEGER")
            + "," + quote(o.getTable()));
    ftype.static_();
    ...
    for (size_t i = 0; i < o.fields.size(); i++)
    {
        ...
        ftype.static_();
        Variable field(fld.name, "Field<" + fld.getCPPType() + ">");
        cl.variable(ftype);
        cl.variable(field);
...
static void writeObjConstructors(Class& cl, const xml::Object& o)
{
    ...
    cons1.param(Variable("db", "const Database&")).constructor(consParams);
    if (hasDefaults)
        cons1.body("defaults();");

    Method cons2(o.name); // Object(const Database &, const Record& row
    cons2.param(Variable("db", "const Database&"))
        .param(Variable("rec", "const Record&"))
    ...
    if (o.fields.size() > 0)
    {
        ...
        for (int i = o.fields.size() - 1; i >= 0; i--)
        {
            ...
            cons2
                .body("case " + toString(p + 1) + ": " + o.fields[i]->name +
                      " = convert<const std::string&, " +
                      o.fields[i]->getCPPType() + ">(rec[" + toString(p) +
                      "]);")
...
static void writeObjRelationHandles(Class& cl, xml::Object& o)
{
    for (size_t i = 0; i < o.handles.size(); i++)
    {
        ...
        Class hcl(className, "RelationHandle<" + o.name + ">");
        Method cons(className);
        cons.param(Variable("owner", "const " + o.name + "&"))
            .constructor("RelationHandle<" + o.name + ">(owner)");
        ...
        Variable exprParam("expr", "const Expr&", "Expr()");
        Variable srcExprParam("srcExpr", "const Expr&", "Expr()");
        Method del("del", "void");
        params.clear();
        params.push_back("owner->getDatabase()");
        params.push_back("expr && " + rel->getName() +
                         "::" + handle.relate->fieldTypeName + " == owner->id");
        del.param(exprParam).body(rel->getName() + "::del(" +
                                  join(params, ", ") + ");");
        ...
        if (handle.destObjects.size() == 1)
        {
            ...
            Method get("get", "DataSource<" + dest->name + ">");
        ...
        else
        {
            if (rel->sameTypes() <= 2)
            {
                Method getTpl("get", "DataSource<T>");
                getTpl.template_("class T").defineOnly().param(exprParam).param(
                    srcExprParam);
                hcl.method(getTpl);
                for (size_t i2 = 0; i2 < handle.destObjects.size(); i2++)
                {
                    ...
                    params.push_back("(" + rel->getName() +
                                     "::" + handle.relate->fieldTypeName +
                                     " == owner->id)" + extraExpr);
                    get.body("return " + rel->getName() +
                             "::" + relate->getMethodName +
                             brackets(join(params, ", ")) + ";");
                    hcl.method(get);
                }
            }
        ...
        Method getRows("getRows", "DataSource<" + rel->getName() + "::Row>");
        getRows.param(exprParam).body("return " + rel->getName() +
...
static void writeObjBaseMethods(Class& cl, const xml::Object& o)
{
    ...
    insert.protected_()
        .param(Variable("tables", "Record&"))
        .param(Variable("fieldRecs", "Records&"))
        .param(Variable("valueRecs", "Records&"))
        .body("tables.push_back(table__);")
        .body("Record fields;")
        .body("Record values;");

    if (o.parentObject.get())
        insert.body("fields.push_back(\"id_\");").body("values.push_back(id);");

    for (Field::sequence::const_iterator f = o.fields.begin();
         f != o.fields.end(); f++)
    {
    ...
    if (o.parentObject.get())
    {
        ...
        insert.body("return " + o.inherits +
                    "::insert(tables, fieldRecs, valueRecs, " + "sequence__);");

    Method create("create", "void");
    create.protected_();
    create.body("Record tables;")
        .body("Records fieldRecs;")
        .body("Records valueRecs;")
        .body("type = type__;")
    ...
    addIDUpdates.protected_().virtual_().param(Variable("updates", "Updates&"));
    if (o.parentObject.get())
    {
        addIDUpdates.body("prepareUpdate(updates, table__);")
    ...
    getFieldTypes.static_().param(
        Variable("ftypes", "std::vector<FieldType>&"));
    if (o.parentObject.get())
    ...
    if (o.parentObject.get())
        update.body("    else").body("        addIDUpdates(updates);");

    update.body("}").body("Persistent::update(updates);").body("oldKey = id;");

    Method delRecord("delRecord", "void");
    delRecord.protected_().virtual_().body("deleteFromTable(table__, id);");
    if (o.parentObject.get())
         ...
         i != o.relations.end(); i++)
    {
        ...
        for (size_t i2 = 0; i2 < relates.size(); i2++)
            params.push_back("(" + rel->getName() +
                             "::" + relates[i2]->fieldTypeName + " == id)");
        delRelations.body(rel->getName() + "::del(*db, " +
                          join(params, " || ") + ");");
    }
    ...
    if (!childrenNames.empty())
    {
        ...
        for (size_t i = 0; i < childrenNames.size(); i++)
        {
            upcast.body("if (type == " + childrenNames[i] + "::type__)")
                .body("    return unique_ptr<" + o.name + ">(new " +
                      childrenNames[i] + "(select<" + childrenNames[i] +
                      ">(*db, Id == id).one()));");
    ...
    for (size_t i = 0; i < o.fields.size(); i++)
    {
        upcastCopy.body("np->" + o.fields[i]->name + " = " + o.fields[i]->name +
                        ";");
    }
    upcastCopy.body("np->inDatabase = inDatabase;")
        .body("return unique_ptr<" + o.name + ">(np);");

    upcast.body("return unique_ptr<" + o.name + ">(new " + o.name +
                "(*this));");
...
{
    Variable table("table__", "const std::string", quote(r.getTable()));
    table.static_();
    cl.variable(table);
    ...
    {
        Variable ftype(
            r.related[i2]->fieldTypeName, "const FieldType",
            quote(r.related[i2]->fieldName + "_") + "," + "A_field_type_" +
                toAttributeString(A_field_type_integer) + "," + "table__");
        ftype.static_();
        cl.variable(ftype);
    }
    for (size_t i2 = 0; i2 < r.fields.size(); i2++)
    ...
    Class rowcl("Row");
    Method rowcons("Row");
    rowcons.param(Variable("db", "const Database&"))
        .param(Variable("rec", "const Record&", "Record()"));
    //        .constructor("Record(db, rec)");
    ...
    for (int i = r.related.size() - 1; i >= 0; i--)
    {
        ...
        Variable fld(fname, "Field<int>");
        rowcl.variable(fld);
        rowcons.body("case " + toString(fieldNum) + ":")
            .body("    " + fname + " = rec[" + toString(fieldNum - 1) + "];");

        consParams.push_back(fname + "(" + r.getName() +
                             "::" + rel.fieldTypeName + ")");
        fieldNum--;
    }
    rowcons.body("}");
...
static void writeRelMethods(const xml::DatabasePtr& database, Class& cl,
                            xml::Relation& r)
{
    Variable dbparam("db", "const Database&");
    Variable destExpr("expr", "const Expr&", "Expr()");
    Variable srcExpr("srcExpr", "const Expr&", "Expr()");
    Method link("link", "void");
    Method unlink("unlink", "void");
    Method del("del", "void");
    Method getRows("getRows", "DataSource<" + r.getName() + "::Row>");

    link.static_().param(dbparam);
    link.body("Record values;").body("Record fields;");
    ...
    {
        link.body("fields.clear();").body("values.clear();");
        for (size_t i = 0; i < r.related.size(); i++)
        {
            xml::Relate& rel = *r.related[i];
            link.body("fields.push_back(" + rel.fieldTypeName + ".name());");
    ...
    {
        xml::Relate& rel = *r.related[i];
        unlinks.push_back(rel.fieldTypeName + " == o" + toString(i) + ".id");
    }
    for (size_t i = 0; i < r.fields.size(); i++)
    {
        xml::Field& fld = *r.fields[i];
    ...
    if (r.isUnidir() == false && r.related.size() == 2 && r.sameTypes() == 2)
    {
        ...
        {
            xml::Relate& rel = *r.related[i];
            unlinks.push_back(rel.fieldTypeName + " == o" + toString(1 - i) +
                              ".id");
        }
        for (size_t i = 0; i < r.fields.size(); i++)
        {
    ...
    getRows.static_().param(dbparam).param(destExpr).body("SelectQuery sel;");
    ...
    {
        xml::Relate& rel = *r.related[i2];
        Variable obj("o" + toString(i2),
                     "const " + database->nspace + "::" + rel.objectName + "&");
        link.param(obj);
        unlink.param(obj);
    }
    for (size_t i2 = 0; i2 < r.fields.size(); i2++)
    {
        ...
        link.param(
            Variable(fld.name, fld.getCPPType(), fld.getQuotedDefaultValue()));
    ...
    if (r.sameTypes() == 1)
    {
        ...
        {
            xml::Relate& rel = *r.related[i2];
            Method get("get", "DataSource<" + database->nspace +
                                  "::" + rel.objectName + ">");
            ...
            get.static_()
                ...
                .body("return DataSource<" + database->nspace +
                      "::" + rel.objectName + ">(db, " + database->nspace +
                      "::" + rel.objectName + "::Id.in(sel) && expr);");
            cl.method(get);
        }
    }
    else
    {
        ...
        for (size_t i2 = 0; i2 < r.related.size(); i2++)
        {
            ...
            Method get(rel.getMethodName, "DataSource<" + database->nspace +
                                              "::" + rel.objectName + ">");
...
{
    vector<string> rec;
    mtd.body("std::vector<Database::SchemaItem> res;");

    mtd.body("std::string TEXT = backend->getSQLType(A_field_type_string);");
    mtd.body("std::string rowIdType = backend->getRowIDType();");

    rec.push_back(quote("schema_"));
    rec.push_back(quote("table"));
    vector<string> r2;
    r2.push_back(quote("CREATE TABLE schema_ (name_ "));
    r2.push_back("TEXT");
    r2.push_back(quote(", type_ "));
    r2.push_back("TEXT");
    r2.push_back(quote(", sql_ "));
    r2.push_back("TEXT");
    r2.push_back(quote(")"));
    ...
    for (size_t i = 0; i < db->sequences.size(); i++)
    {
        ...
        rec.push_back("backend->getCreateSequenceSQL(" +
                      quote(db->sequences[i]->name) + ")");
        mtd.body("    res.push_back(Database::SchemaItem(" + join(rec, ",") +
                 "));");
    }
    mtd.body("}");
    ...
    std::cout << toString(db->tables.size()) + " tables\n" << std::endl;
    std::cout << toString(db->sequences.size()) + " sequences\n" << std::endl;
    std::cout << toString(db->indices.size()) + " indices\n" << std::endl;
}
...
void CppGenerator::writeCPPClasses(const ObjectModel* model)
{
    ...
    fprintf(hpp, "#include <casket/storage/storage.hpp>\n");
    ...
    fprintf(cpp, "using namespace gen;\n");

    std::cout << "writing prototypes for Persistent classes\n" << std::endl;
    for (size_t i = 0; i < model->objects.size(); i++)
        fprintf(hpp, "class %s;\n", model->objects[i]->name.c_str());

    std::cout << "writing relations\n" << std::endl;
    for (Relation::sequence::const_iterator it = model->relations.begin();
         it != model->relations.end(); it++)
    {
        Class cl((*it)->getName());
    ...
    std::cout << "writing persistent objects\n" << std::endl;
    for (ObjectSequence::const_iterator it_o = model->objects.begin();
         it_o != model->objects.end(); it_o++)
    {
    ...
    std::cout << "writing database class" << std::endl;
...
{
    return (m_sources_directory.empty()) ? getOutputDirectory()
                                         : m_sources_directory;
}

const std::string& CppGenerator::getOutputIncludesDirectory() const
{
    return (m_includes_directory.empty()) ? getOutputDirectory()
                                          : m_includes_directory;
}

std::string
CppGenerator::getOutputSourcesFilename(const std::string& name) const
{
...
std::string
CppGenerator::getOutputIncludesFilename(const std::string& name) const
{
...
bool CppGenerator::generateCode(const ObjectModel* model)
{
    ...
    string hppName =
        getOutputIncludesFilename(toLower(model->db->name) + ".hpp");
    ...
    string cppName =
        getOutputSourcesFilename(toLower(model->db->name) + ".cpp");
